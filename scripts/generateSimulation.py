...
def generateSimulation(numRandom, numFundamental, numMao,  numMomentum, numQLearning, numDQL, duration=10000, startingPrice=10000, output="simulations/GeneratedSimulation.xml"):
    ...
    half = numMao // 2
    for i in range(half):
        ET.SubElement(root, "MAOAgent", name=f"TRADER_MAO_{i:02d}", exchange="MARKET", marketDataAgent="MARKET_DATA_AGENT_SMALL")

    for i in range(half):
        ET.SubElement(root, "MAOAgent", name=f"TRADER_MAO_{i + half:02d}", exchange="MARKET", marketDataAgent="MARKET_DATA_AGENT_LARGE")

    for i in range(numMomentum):
        ET.SubElement(root, "MomentumAgent", name=f"TRADER_MOMENTUM_{i:02d}", exchange="MARKET")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--momentum", type=int, default=0, help="Number of Momentum agents")
